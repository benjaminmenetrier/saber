...
void BifourierAromeBalance::read() {
  ...
  if (params_.read.value()->inputFileFormat.value() == "arome legacy binary"
    ...
    for (size_t jm = 0; jm < balance_->trans()->ellips().size(); ++jm) {
      kspec2g += 4*(balance_->trans()->ellips()[jm]+1);
    }
    ...
    attributes.set("nsmax", balance_->trans()->nwGlb()-1);
    ...
    if (balance_->comm().rank() == 0) {
      ...
      if (params_.read.value()->inputFileFormat.value() == "arome legacy binary") {
        ...
        ASSERT(nsmaxp1File == balance_->trans()->nwGlb());
...
void BifourierAromeBalance::write() const {
  ...
  if (params_.write.value()) {
    ...
    if (params_.write.value()->outputFileFormat.value() == "arome legacy binary"
      ...
      // Define attributes
      eckit::LocalConfiguration attributes;
      const atlas::StructuredGrid & outerGrid = balance_->trans()->geometryData().functionSpace().grid();
      const atlas::StructuredGrid & gpGrid = balance_->trans()->gpFspace().grid();
      ...
      attributes.set("nsmax", balance_->trans()->nwGlb()-1);
      attributes.set("nmsmax", balance_->trans()->M());
      attributes.set("nflev", nz_);
      attributes.set("kspec2g", kspec2g);
