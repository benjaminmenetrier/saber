...
std::tuple<oops::Variables, oops::Variables> SaberParametricBlockChainGroup::initCentralBlock(
  ...
  const oops::Variables currentOuterVars = outerBlockChain_ ?
                             outerBlockChain_->innerVars() : chainVars_;
...
void SaberParametricBlockChainGroup::testCentralBlock(
        const eckit::LocalConfiguration & covarConf,
        const SaberBlockParametersBase & saberCentralBlockParams,
        const oops::GeometryData & outerGeom,
...
void SaberParametricBlockChainGroup::filter(oops::FieldSet4D & fset4d) const {
...
void SaberParametricBlockChainGroup::multiply(oops::FieldSet4D & fset4d) const {
...
void SaberParametricBlockChainGroup::randomize(oops::FieldSet4D & fset4d) const {
...
size_t SaberParametricBlockChainGroup::ctlVecSize() const {
  if (crossTimeCov_) {
    // Duplicated cross-time covariances
    if (timeComm_.rank() == 0) {
...
void SaberParametricBlockChainGroup::multiplySqrt(const atlas::Field & cv,
...
void SaberParametricBlockChainGroup::multiplySqrtAD(const oops::FieldSet4D & fset4d,
                                                    atlas::Field & cv,
                                                    const size_t & offset) const {
  // Initialization
